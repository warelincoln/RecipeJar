...
| **0.1b Dev/Prod Isolation** | In progress — app split done, dev Supabase next | Debug and Release iOS apps are already isolated (`app.orzo.ios.dev` vs `app.orzo.ios`). Next step is a separate Supabase project for dev so schema, auth, storage, and migration testing never touch production. Separate Railway service remains optional for now. |
...
| **Status** | **In progress** — Pillar 4 complete (separate "Orzo Dev" app). Pillar 2 (separate dev Supabase) is now the recommended next step. Pillars 1 and 3 remain optional for solo development. |
...
**What to build next (Pillar 2 — separate Supabase for dev):**

- [ ] Create a second Supabase project dedicated to development
- [ ] Point local `server/.env` at the dev Supabase project instead of production
- [ ] Keep Railway pointed at production Supabase only
- [ ] Mirror required schema, RLS policies, buckets, and storage conventions into the dev project
- [ ] Configure dev auth redirect URLs and Site URL for `app.orzo.ios.dev://auth/callback`
- [ ] Decide whether Apple/Google Sign-In should be configured in dev immediately or deferred in favor of email/password-only dev auth
- [ ] Seed the dev project with safe test data
- [ ] Rehearse future schema migrations against dev Supabase before touching production
