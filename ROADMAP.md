...
| Date | Change |
|---|---|
| 2026-04-16 (afternoon) | Cross-cutting recipe UX upgrades shipped off-roadmap (four commits: `5c04b97`, `750bf88`, `2a19180`, `51b2e04`). **PR A:** source provenance chip on detail, prep/cook/total time columns (migrations 0013 + 0014) with AI inference + TimesReviewBanner, derived total fallback, ½/2×/3× servings quick chips. **PR B:** iOS-Photos-style bulk select mode on Home + Collection (long-press entry, checkmark overlay, floating BulkActionsBar, server `POST /recipes/bulk-delete` + `PATCH /recipes/bulk-collection`, haptics via `react-native-haptic-feedback`). **Polish:** inline `+ New folder` in CollectionPickerSheet (replaces dead-end alert for zero-collection users), correct single-recipe title in bulk delete confirm, FAB hidden in bulk mode, stronger haptics. **Fix:** press-after-longpress guard so the card that entered bulk mode stays selected. Consciously pushes Phase 0.2 TestFlight by ~3–4 days; trade made for App Store screenshot polish. See `CHANGELOG.md` 2026-04-16 entries for full detail; `docs/CURRENT_STATE.md` for how it slots against each phase; `docs/STATUS.md` for proven-on-device evidence. |
