...
## Current State Assessment (as of 2026-03-31)
...
- Supabase used exclusively for file/image storage (NOT for database queries — server connects to Postgres directly via `postgres` driver + Drizzle ORM)
- OpenAI GPT-5.4 for Vision and text extraction
- 8 database migrations (0000–0007)

### What's NOT Built (every roadmap feature from Phases 0–5)

| Feature | Status | Notes |
|---|---|---|
| **0.1 Auth** | Not started | No users table, no user_id columns anywhere, no JWT middleware, no login/signup screens, no RLS. Entirely single-user. |
...
- **Supabase:** Used exclusively for file storage (two public buckets: `recipe-pages` for draft images, `recipe-images` for hero/thumbnail). Client uses service role key (bypasses all RLS).
- **Implication:** If the Postgres instance IS Supabase-hosted, the Supabase Auth + RLS plan is viable — but it requires: (1) adding `user_id` FK to every content table, (2) writing RLS policies, (3) adding JWT verification middleware to Fastify, (4) building mobile auth screens, and (5) migrating all existing data to a seed user. If the Postgres is NOT Supabase-hosted, the auth strategy needs rethinking.
