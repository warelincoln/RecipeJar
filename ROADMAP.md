...
|---|---|---|
| **0.1 Auth** | **Mostly complete — WS-1/2/3/4/5 done; WS-6/7/8 pending** | Server: `profiles` table, `user_id` on 4 domain tables, JWT auth middleware, user-scoped repositories, RLS (41 policies). Mobile: Supabase client with Keychain, auth-gated navigation, onboarding carousel, Apple/Google/email sign-in, password reset deep link, account screen with sign-out. All three auth methods tested on physical iPhone. **Remaining (non-blocking for TestFlight):** WS-6 (storage bucket privacy — private buckets + signed URLs), WS-7 (session lifecycle, MFA enrollment UI, account deletion), WS-8 (rate limiting, audit logging, integration tests). See `docs/AUTH_RLS_SECURITY_PLAN.md`. |
| **0.2 Ship Prep & TestFlight** | Not started | No App Store Connect listing, no provisioning profiles, no analytics SDK, no crash reporting, no privacy policy. |
...
- **Supabase Storage:** Two **public** buckets (`recipe-pages`, `recipe-images`). Server uses service role key. **WS-6 (pending)** will convert these to private buckets with signed URLs and user-scoped paths.
- **Mobile auth (COMPLETE — WS-4):** Supabase client installed on mobile with Keychain session storage. Auth-gated navigation (onboarding → auth screens → main app). Apple Sign-In (SHA-256 nonce), Google Sign-In (with `iosClientId`), and email/password with verification all functional. `api.ts` sends Bearer tokens on all requests with single-flight refresh. Account screen with sign-out. Password reset via deep link. Tested end-to-end on physical iPhone.
- **Remaining work:** WS-6 (storage privacy — private buckets + signed URLs), WS-7 (session lifecycle/MFA flows/account deletion), WS-8 (abuse controls/tests). None of these block TestFlight for trusted testers. See `docs/AUTH_RLS_SECURITY_PLAN.md`.
...
| **Status** | **WS-1/2/3/4/5 complete. WS-6/7/8 pending (non-blocking for TestFlight).** |
...
- Redirect URL: `app.recipejar.ios://auth/callback`
...
**What remains to build (by work stream):**

- **WS-6 — Storage Security (can now proceed — WS-4 dependency satisfied):** Convert `recipe-pages` and `recipe-images` buckets from public to private, implement user-scoped storage paths (`{userId}/...`), replace `getPublicUrl()` with `createSignedUrl()` (60-min TTL), validate userId in upload paths, migrate existing storage objects to user-scoped paths
- **WS-7 — Session Management & Recovery:** Session list/revoke endpoints, sign-out-all, step-up auth (10-min reauthentication window for sensitive actions), email change flow, MFA TOTP enrollment + backup codes in the app, provider linking (Apple ↔ Google ↔ email), account deletion with 30-day soft delete. Note: password reset flow is already built (ForgotPasswordScreen + ResetPasswordScreen + deep link handler).
- **WS-8 — Abuse Controls & Testing:** Rate limiting per surface (login, signup, reset), CAPTCHA escalation on repeated failures, audit event logging, user notifications for high-value events, Vitest integration tests for auth/IDOR/RLS/session/recovery, manual security checklist

**None of WS-6/7/8 block TestFlight for trusted testers.** The next blocker is deploying the Fastify server to a cloud host (currently runs on localhost).

**Migration strategy for existing data:** ✅ Complete. Seed user (`migration-seed@recipejar.app`, banned) owns all 211 pre-auth rows.
...
```
0.1 Auth ──────────────┬──→ 0.2 Ship Prep / TestFlight
                       │         │
