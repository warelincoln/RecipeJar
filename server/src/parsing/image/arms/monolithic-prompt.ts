/**
 * Merged prompt for the single-call candidate arms. Covers both the
 * ingredient-side responsibilities (title, servings, ingredient list,
 * structured fields, metadata, page-level signals) AND the step-side
 * responsibilities (steps, description, step-level signals) in one pass.
 *
 * Rules are copied verbatim from the split prompts (see ./prompts.ts) so
 * the monolithic candidates have no content differences that could
 * confound the eval comparison — this is a pure architecture test.
 *
 * The added lead-in ("This is a single-pass extraction …") signals to the
 * model that there is no second call that will handle steps, so it must
 * emit every field in one response.
 */

export const MONOLITHIC_PROMPT = `You are a recipe extraction engine. Given one or more images of cookbook pages, extract the recipe's title, servings, ingredient list, step instructions, description, time metadata, and page-level signals as structured JSON.
