...
- **Image parsing:** Edit `server/src/parsing/image/image-parse.adapter.ts`. **Current architecture (shipped 2026-04-21):** single `gpt-4o` Vision call with merged JSON schema (`schemas.ts:recipeSchema`) and merged prompt (`prompts.ts:RECIPE_PROMPT`), `temperature: 0`, `max_completion_tokens: 4500`, `detail: "high"`. The prompt is signal-rich: includes `ingredientSignals`, `stepSignals`, and top-level signal hints for OCR quality detection. Single call replaces the 2026-04-19 split-call after a 4-arm eval trade study (see `ARCHITECTURE.md` Image Parse section for the comparison). To change the model, update the `MODEL` constant in `image-parse.adapter.ts` AND the pricing table in `pricing.ts` — the adapter emits cost events per call so mismatched pricing silently shows wrong numbers in PostHog. Images are sent as base64 data URLs (downloaded from Supabase at parse time, resized through `optimizeForUpload`, encoded inline) to avoid an extra network hop for OpenAI.
- **Image optimization:** Edit `server/src/parsing/image/image-optimizer.ts`. Core paths: `optimizeForUpload` (auto-orient, resize ≤3072px, JPEG 85% — draft page storage + what gets sent to OpenAI Vision). Additional helpers produce **hero** and **thumbnail** JPEGs for saved recipe images (`recipe-images` bucket). Uses `sharp`. The previous `optimizeForOcr` re-encode at 3072 @ q90 was removed 2026-04-19 as redundant. Classical OCR preprocessing (grayscale, CLAHE, sharpen) was tested and found to degrade OpenAI Vision accuracy. The 3072px resolution is required for accurate fraction reading (⅓ vs ½); 2048px caused consistent misreads during earlier experiments. **Queued follow-up (2026-04-21):** the WYSIWYG camera fix means users now frame tighter shots — revisit whether 2048px would pass the eval fixtures now that text fills more of the captured pixels. Potential further cost reduction.
