...
- **`SERVINGS_MISSING` display copy is stale** (ships with Build 2): server-side severity was downgraded from BLOCK to FLAG (`7733165`), but the mobile display message in `mobile/src/features/import/issueDisplayMessage.ts:48` still reads *"How many does this recipe serve? Add the number of servings so you can save."* — that phrasing implies save is blocked, which it isn't anymore. Rewrite to something like *"How many does this recipe serve? You can add it now or skip and save anyway."* on the next mobile build.
- **Home-screen image flicker on return visits** (task chipped, hot-deployable): `GET /recipes` generates fresh signed URLs on every call, so FastImage sees a new URL string each time the user returns to home and re-renders. Fix: in-memory server-side cache in `server/src/services/recipe-image.service.ts` that returns the same signed URL for ~55 min per path. Task brief with full spec exists; implementation is ~30 lines and ships via Railway.
- **Build 2 follow-up — FastImage `cacheKey` prop**: companion to the server cache above. Passing a stable cache key (the storage path) to FastImage would make the disk cache URL-independent — even a server cache bust wouldn't cause reloads. ~5 lines in the component that renders recipe cards.
