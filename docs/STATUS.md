...
- **Rule-based time fallback for partial JSON-LD**: When JSON-LD publishes prepTime + cookTime but no totalTime (e.g. savoryonline), the client derives `~total` at render time. A server-side "gap-fill" AI pass for JSON-LD-wins-but-incomplete-times cases was scoped but not built. Low priority since client fallback covers the common case.
