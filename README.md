...
- **Recipe times (prep / cook / total) with AI inference**: every recipe stores `prepTimeMinutes`, `cookTimeMinutes`, `totalTimeMinutes` (nullable integers) plus a provenance column for each (`prep_time_source` / `cook_time_source` / `total_time_source`, values `"explicit" | "inferred" | "user_confirmed" | null`). JSON-LD + Microdata times are auto-tagged `"explicit"`; the Vision + URL-AI prompts may produce `"inferred"` estimates when the source doesn't state a time. AI-inferred unconfirmed values render italic with a `~` prefix on detail; the import preview shows a **TimesReviewBanner** that lets the user accept or edit estimates (flips to `"user_confirmed"`). Post-save edit has an auto-sum that derives total from prep + cook when total is empty (manual total wins). When the stored total is null but prep+cook are set, the detail chip derives a `~Xm total` client-side so sites that supply only prep+cook (common JSON-LD gap) still render a usable total.
