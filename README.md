...
- `server/drizzle/0008_auth_profiles_user_id.sql` through `server/drizzle/0011_user_sessions.sql`
  - Migration 0008: profiles table, auto-create trigger, user_id columns, indexes. Migration 0009: RLS enabled on all tables. Migration 0010: MFA recovery codes table. Migration 0011: user sessions table.
...
- **DANGER: `server/.env` and Railway share the same database credentials.** The `DATABASE_URL` password in `server/.env` (local dev) and in Railway's environment variables (production) must both be valid for the same Supabase database. **Never reset the Supabase database password** to fix a local issue — this instantly breaks the production Railway deployment. If `server/.env` has a stale password, ask for the current password and update the `.env` file to match. If you must reset the password, update **both** `server/.env` **and** Railway's `DATABASE_URL` variable immediately, then redeploy Railway. After any credential change, restart the local dev server (`npm run dev:phone`) and verify the connection before reporting it fixed.
...
- **C3: Apple client secret expiry** — ES256 JWT expires ~October 2026. Tracked in `docs/SECURITY_CHECKLIST.md`.

**Reference:** `docs/AUTH_RLS_SECURITY_PLAN.md` (architecture), `docs/SECURITY_CHECKLIST.md` (manual audit), `docs/PRODUCTION_DEPLOY.md` (deployment).
...
- **2026-04-14** — **Mobile app terracotta palette migration complete.** Created `mobile/src/theme/colors.ts` canonical palette module with raw tokens (`TERRACOTTA`, `ESPRESSO`, `SAGE_GREEN`, `PAPRIKA`, etc.) + semantic aliases (`PRIMARY`, `TEXT_PRIMARY`, `ERROR`, `DIVIDER`, etc.). Added `MUTED_PLUM` and `DUSTY_ROSE` tokens for soft food-semantic icon variety. 37 files migrated across screens/features/components (846+/569- lines). `collectionIconRules.ts` bright Tailwind colors softened to muted palette variants (greens→`SAGE_GREEN`, reds→`PAPRIKA`, pinks→`DUSTY_ROSE`, purples→`MUTED_PLUM`); food-semantic warm inline hexes (oranges, browns) preserved. Jar fan actions now use four contrasting palette colors (Camera=`GOLDEN_AMBER`, Photos=`DUSTY_ROSE`, URL=`SAGE_GREEN`, Add Folder=`MUTED_PLUM`). Jar FAB "+" migrated from `#fb923c` to `PRIMARY` terracotta. Details in changelog.
- **2026-04-10** — **Brand refresh & landing page launched.** New app icon: stylized cream orzo pasta grains forming an "O" on a terracotta background (replaces the blue amphora). New warm color palette (terracotta `#C4633A`, cream `#FFF8F0`, espresso `#2D1F14` — see `ROADMAP.md` → "Brand Identity & Color Scheme"). All 8 iOS icon sizes regenerated. Landing page built at `landing/` (vanilla HTML + inline CSS/JS, no build step): hero with email signup, 3-step "How it works", 6 feature cards (Snap & Digitize, Save from Any App, Verified Not Guessed, Smart Scaling, Meal Planning & Grocery Lists, Cook Mode), differentiator section, privacy policy, terms of service, Google Analytics (`G-9RJ2D98SZW`). Tagline: "Your cookbook, upgraded." Email waitlist wired to Klaviyo (company ID `TU2v9j`, list ID `X2KYb2`) via client-side API. Deployed to Cloudflare Pages at `orzo-website.pages.dev` (custom domain `getorzo.com` pending).
...
- **2026-03-31** — **Servings, structured ingredients & dynamic scaling.** `baselineServings`, structured ingredient fields, deterministic ingredient parser, `SERVINGS_MISSING` BLOCK rule, client-side scaling. Migration `0007`. Details in changelog.
- **2026-03-30** — **Collection folder rename & delete, concurrent import queue, recipe hero images.** Details in changelog.
- **2026-03-28** — iOS patches, draft API field names, preview reveal. Details in changelog.
- **2026-03-25** — Photo library import, WebView URL import, clipboard prompt. Details in changelog.
- **2026-03-22** — Image optimization, user notes, star rating, collections overhaul, auto icons, URL parsing. Details in changelog.
- **2026-03-21** — Collections, recipe editing, validation simplification, Lucide icons, iOS UI tests, dev workflow. Details in changelog.
- **2026-03-20** — Import flow fix, UX improvements. Details in changelog.
