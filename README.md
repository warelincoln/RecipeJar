...
- `mobile/src/screens/HomeScreen.tsx`
  - Jar fan actions (Camera=`GOLDEN_AMBER`, Photos=`DUSTY_ROSE`, URL=`SAGE_GREEN`, Add Folder=`MUTED_PLUM`), Photos picker, photo preview screen, button styling, FAB auto-open for concurrent imports; **long-press** folder chips (not the virtual **All Recipes** chip) for rename/delete. **Long-press a recipe card enters iOS-Photos-style bulk select mode** (see `useBulkSelection` below) — header swaps to `Cancel / "N selected" / Select All`, `BulkActionsBar` slides up, title/search/collections/FAB hide, and the grid pads its bottom so the bar doesn't truncate the last row. `recentLongPressRef` swallows the spurious `onPress` that fires on some iOS devices right after `onLongPress` for the same card
...
- `mobile/src/screens/CollectionScreen.tsx`
  - Folder **`MoreHorizontal`** menu (rename / delete), **`CreateCollectionSheet`** rename mode, **`DeleteCollectionConfirmSheet`**, **404** handling when folder was deleted elsewhere. **Shares the bulk select mode with HomeScreen via `useBulkSelection`** — primary action is `"Remove from folder"` inside a specific collection (bulk null-assign), and `"Add to collection"` on the All Recipes view
...
- `server/src/api/recipes.routes.ts`
  - Assign-to-collection validates collection exists (**404** if missing). New **`POST /recipes/bulk-delete`** body `{ids: string[]}` → `{deletedCount}` and **`PATCH /recipes/bulk-collection`** body `{ids, collectionId: string | null}` → `{updatedCount}` (both return JSON — NOT 204 — because the mobile `request()` helper calls `.json()` on every response). **`enrichRecipeResponse`** wrapper now constructs the `sourceContext` object the client expects, resolving signed URLs for `recipe-pages` bucket thumbnails via `resolveSourcePageUrl` — the `sourceContext` field was previously declared in the shared `Recipe` type but **not actually emitted by the server** (latent dead code)
...
- `server/src/parsing/time.ts` (+ `server/tests/time.test.ts`)
  - `isoDurationToMinutes("PT1H30M") → 90`; null / malformed / sub-minute → `null`. Called from the save path in `drafts.routes.ts` to derive minute values from `ParsedRecipeCandidate.metadata.{prepTime,cookTime,totalTime}` ISO 8601 strings before persisting. 10 passing Vitest cases covering hours, minutes, seconds-rounding, leading date components, non-string inputs
- `server/src/parsing/image/image-parse.adapter.ts` + `server/src/parsing/url/url-ai.adapter.ts`
  - Parse-time prompts emit `{prepTime, prepTimeSource, cookTime, cookTimeSource, totalTime, totalTimeSource}` where each parse-level `*TimeSource` is `"explicit"` (literally stated on the source) / `"inferred"` (AI estimated from content) / `null`. JSON-LD + Microdata extractions in `url-structured.adapter.ts` are auto-tagged `"explicit"`. The DB-level [`TimeSource`](shared/src/types/recipe.types.ts) union adds `"derived"` (server arithmetic sum of explicit prep+cook) and `"user_confirmed"` (banner accept or post-save edit); the save handler persists `prep_time_source` / `cook_time_source` / `total_time_source` accordingly.
- `server/src/parsing/url/url-dom-enrichment.ts`
  - Top-up pass that fills missing `metadata.imageUrl` (og:image / twitter:image / link rel=image_src), `servings` (WPRM / Tasty / itemprop / regex yield patterns), and **`totalTime`** (recipe-scoped DOM scan for `"READY IN X"` / `"Total: X"` / `"Total time: X"` labels, shipped 2026-04-22) when structured data omitted them. Never overwrites. `enrichedTotalTime` flag logged at all 5 extraction sites for PostHog observability.
...
- `mobile/src/utils/time.ts`
  - Time formatting + parsing helpers: `formatMinutes(90) → "1h 30m"`, `hasAnyTime(prep, cook, total)`, `isoDurationToMinutes("PT1H30M") → 90`. Mirror of the server helper — used by detail/edit screens and the import preview's times review banner
- `mobile/src/screens/RecipeDetailScreen.tsx`
  - Hero, source provenance block (URL hostname chip → Safari OR photo thumbnail strip → `FullScreenImageViewer`), time chip row (`~Xm prep · Ym cook · Zm total`, italic+tilde for AI-inferred unconfirmed values, clean for explicit / user-confirmed, derived total fallback when prep+cook are set but total is null), rating, verified badge, **servings quick chips (½ / 2× / 3×)** above the existing stepper, ingredients, steps, notes, delete. All source/time fields read from `recipe.sourceContext` + `recipe.{prepTime,cookTime,total}TimeMinutes` + `recipe.{prep,cook,total}TimeSource`
- `mobile/src/screens/RecipeEditScreen.tsx`
  - Post-save edit: title, description, ingredients, steps, collection, servings, hero photo, **Times (minutes, optional)** with auto-sum prep+cook→total (manual total wins; `totalIsAutoFilled` flag tracks which is which; label reads "Total (auto)" while auto-filled)
- `mobile/src/features/import/PreviewEditView.tsx`
  - Pre-save preview with the **TimesReviewBanner** that renders when at least one time was AI-inferred (reads `parsedMetadata.{prepTimeSource,cookTimeSource,totalTimeSource}`). Three editable fields pre-seeded with the estimates + "Accept estimates" button; editing or accepting writes user-override values to `EditedRecipeCandidate.{prepTimeMinutes,cookTimeMinutes,totalTimeMinutes}` via `onEdit` → `PATCH /drafts/:draftId/candidate`. Save path picks up those overrides and persists `source = "user_confirmed"`
- `mobile/src/components/BulkActionsBar.tsx`
  - Floating bottom action bar for bulk-select mode. `Animated.spring` slide-in/out (mirrors the HomeScreen FAB-fan pattern). Two actions: configurable primary (`"add-to-collection"` → opens `CollectionPickerSheet` / `"remove-from-collection"` → bulk null-assign) + Delete. Disables when `count === 0`. Positions above `insets.bottom + 12`
- `mobile/src/hooks/useBulkSelection.ts`
  - Shared bulk-select state for HomeScreen + CollectionScreen: `bulkMode: boolean`, `selectedIds: Set<string>`, `enterBulk(initialId)` / `toggle(id)` / `selectAll(allIds)` / `clear()` / `exit()`. Fires `haptics.tap()` on entry and `haptics.toggle()` on each toggle
- `mobile/src/services/haptics.ts`
  - Thin wrapper around `react-native-haptic-feedback`. `haptics.tap()` = `impactMedium` (bulk entry), `haptics.toggle()` = `impactLight` (selection). Errors swallowed — haptics are polish, not functional. **Native dep: adding or removing this module requires `cd mobile/ios && pod install` + a Debug rebuild in Xcode**
- `mobile/src/components/CollectionPickerSheet.tsx`
  - Adds a terracotta **`+ New folder`** row at the top of the picker when `onCreateNewCollection` is provided. Parent closes the picker and opens `CreateCollectionSheet`; on save, creates the folder and assigns the selection to it in one user action. Zero-collection users see a picker with only the New folder row — replaces the old "No collections yet" dead-end alert
- `mobile/src/components/RecipeCard.tsx`
  - Optional `bulkMode` + `selected` props. When `bulkMode`, a 26px checkmark circle renders top-right — filled `PRIMARY` with white check when selected, empty white outline over a 25%-opacity scrim when not
- `mobile/src/components/RecipeQuickActionsSheet.tsx` → `RecipeDeleteConfirmSheet`
  - Optional `count?: number` prop. When `> 1` copy reads "Delete N recipes?" with plural details; when `== 1` uses the actual recipe title (looked up from the store on the caller side)
- `mobile/src/components/ToastQueue.tsx` → `ToastItem.onUndo`
  - Now optional. Undo button hidden when omitted (bulk toasts are informational-only since restoring N recipes isn't cheap)
...
- `server/drizzle/0008_auth_profiles_user_id.sql` through `server/drizzle/0015_drafts_resolved_url.sql`
  - Migration 0008: profiles table, auto-create trigger, user_id columns, indexes. 0009: RLS on all tables. 0010: MFA recovery codes. 0011: user sessions. 0012: cascade user data on profile delete. **0013:** `prep_time_minutes`, `cook_time_minutes`, `total_time_minutes`, `description_summary` on `recipes`; `summary_text` on `recipe_steps`. **0014:** `prep_time_source`, `cook_time_source`, `total_time_source` (text) on `recipes` — provenance for AI-inferred vs. explicit vs. user-confirmed times. **0015 (2026-04-24):** `resolved_url` (text, nullable) on `drafts` — persisted when the URL-fallback cascade rescues a non-recipe page, so retries of `POST /drafts/:id/parse` skip link discovery.
...
- **`react-native-haptic-feedback`** is a native module (added 2026-04-16 for bulk-select mode entry/toggle haptics). If bulk-mode haptics don't fire on your build, you're almost certainly on a stale native binary — rebuild Debug in Xcode. If you ever remove/upgrade haptics, the usual `pod install` + Debug rebuild applies.
- **Fresh dev Supabase projects need the `recipe-pages` bucket created manually.** Unlike `recipe-images` (auto-created by `ensureRecipeImagesBucket()` at server startup), `recipe-pages` has no auto-create guard. Symptom: photo import uploads fail with "Upload failed." Fix: in Supabase dashboard → Storage → New bucket → `recipe-pages`, set public = false. The bulk image upload path (`POST /drafts/:id/pages`) will then work.
...
- **Single-call image parsing** (shipped 2026-04-21 after the cost trade study): one OpenAI call per parse. `gpt-4o` Vision at `detail:"high"` + 3072px + `temperature: 0`, merged JSON schema (`response_format: { type: "json_schema", strict: true }`), `max_completion_tokens: 4500`. Single `RECIPE_PROMPT` covers title, servings, ingredients (+ fractions), steps (≤40-word concision), description, metadata, and page-level signals in one pass. Replaced the 2026-04-19 two-call split architecture after a 4-arm eval showed monolithic gpt-4o beats it on cost (-42%: $0.048 → $0.028 per recipe) with equal accuracy and unchanged latency. Claude Sonnet 4.6 and Haiku 4.5 were evaluated and rejected (too slow / too expensive and fails fraction gate respectively). **Per-call cost instrumentation** (`pricing.ts` + `parse_tokens` / `server_parse_tokens` / `server_parse_cost` PostHog events) makes the trade continuously observable. Measured: single-page p50 ~19s (dev) / ~24s (live real-world). **Fraction verification UX**: non-integer ingredient amounts render with a `LIGHT_PEACH` tint + persistent italic note to compensate for residual ~10% deterministic LLM fraction misreads on some cookbook fonts. See [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) → "Image Parse" for the architecture history + eval results. See [`docs/PARSING_KNOWN_LIMITATIONS.md`](docs/PARSING_KNOWN_LIMITATIONS.md) → "Image parsing."
...
- **Servings & ingredient scaling**: every recipe stores `baselineServings`. Ingredients are persisted with structured fields (`amount`, `amountMax`, `unit`, `name`, `raw`, `isScalable`). Parsing extracts these from GPT output (image/URL) or via a **deterministic regex/rules-based ingredient parser** (JSON-LD/microdata strings + Rule A re-parse on saved recipe edit). Missing servings is a `BLOCK`-severity validation issue — the user must specify servings before saving. Detail screen provides an interactive servings stepper **plus ½ / 2× / 3× quick chips** that scale ingredient amounts client-side (mixed-number formatting with unicode fractions, ⅛ rounding, no unit conversion).
- **Recipe times (prep / cook / total) with AI inference**: every recipe stores `prepTimeMinutes`, `cookTimeMinutes`, `totalTimeMinutes` (nullable integers) plus a provenance column for each (`prep_time_source` / `cook_time_source` / `total_time_source`, values `"explicit" | "derived" | "inferred" | "user_confirmed" | null`). JSON-LD + Microdata times are auto-tagged `"explicit"`; the Vision + URL-AI prompts may produce `"inferred"` estimates when the source doesn't state a time. AI-inferred unconfirmed values render italic with a `~` prefix on detail; the import preview shows a **TimesReviewBanner** that lets the user accept or edit estimates (flips to `"user_confirmed"`). Post-save edit has an auto-sum that derives total from prep + cook when total is empty (manual total wins). **Two-layer totalTime gap closer (shipped 2026-04-22):** (a) `enrichFromDom` scans recipe-scoped HTML for `"READY IN X"` / `"Total: X"` labels and populates `metadata.totalTime` as `"explicit"` when structured data omitted it; (b) the save path derives `total = prep + cook` tagged `"derived"` (renders clean, no `~`) when both components are present but neither JSON-LD nor DOM provided a total. Client render-time fallback at `RecipeDetailScreen.tsx:343-358` remains for legacy rows saved before the fix.
- **Source provenance on recipe detail**: every saved recipe exposes `sourceContext: { sourceType, originalUrl, pages: [{id, orderIndex, imageUri}] }` (page URIs resolved to signed URLs by `enrichRecipeResponse`). URL imports render a hostname chip that opens Safari; photo imports render "Imported from photo" + a horizontal thumbnail strip that opens the source page in `FullScreenImageViewer`.
- **Bulk select mode on Home + Collection** — long-press any recipe card to enter iOS-Photos-style multi-select (checkmarks, `Animated.spring` floating action bar, `react-native-haptic-feedback` for entry + toggle taps). Two primary variants: `"add-to-collection"` (Home + All Recipes, opens `CollectionPickerSheet`) or `"remove-from-collection"` (inside a specific collection, bulk null-assign). Delete is always available. All paths are **one server round-trip via `POST /recipes/bulk-delete`** (with app-level cascade + hero-image cleanup) or **`PATCH /recipes/bulk-collection`** (single transaction with collection-ownership validation). Bar disables when 0 selected; grid pads its bottom so nothing is truncated; Jar FAB hides in bulk mode.
- **Inline `+ New folder` in the collection picker**: picker sheet renders a terracotta `+ New folder` row when `onCreateNewCollection` is provided. Zero-collection users go straight from "Add to collection" to creating a folder and assigning in one flow — no more "No collections yet" dead-end alert. Works for both single-recipe flows (`RecipeDetailScreen`) and bulk flows.
...
- **Concurrent import queue** (up to 3 image-based recipes): fire-and-forget `202 Accepted` parse endpoint, server-side parse concurrency semaphore (max 3 parses × 2 parallel OpenAI calls per parse = up to 6 simultaneous requests, bumped 2→3 on 2026-04-19 to match this documented contract), client-side Zustand store with AsyncStorage persistence, exponential-backoff poller, Import Hub screen, app-wide floating banner, enqueue function with retry and orphan cleanup, idempotency guards on parse and save, startup cleanup of stuck/cancelled drafts, draft cancel endpoint with Supabase image cleanup
...
- **URL-fallback cascade (shipped 2026-04-24):** when all primary extraction tiers (JSON-LD → Microdata → DOM-AI) fail on the user-pasted URL, the server runs a two-layer rescue in `tryUrlFallback`: (1) `rel="canonical"` short-circuit with self-loop guard; (2) deterministic scoring over page anchors + JSON-LD `@graph` Recipe URLs, with ambiguity decline when ≥8 candidates cluster within 2 points of the top (roundup-post detection). Depth-capped at 1 retry. `drafts.resolved_url` (migration 0015) persists the pick so retries skip discovery. Review screen renders a disclosure banner naming the destination host + a "Back to browser" link that re-opens the WebView at the user's original URL. See [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) → "URL Parse" for the full scoring table.
- **Import-review two-tab UI (shipped 2026-04-24):** `PreviewEditView` now has a segmented `[ Imported | Source ]` tab row. URL imports show the parsed page in a `WebView` on the Source tab; image imports show a vertical photo stack with tap-to-zoom. Sibling-rendered with `display: none` toggling so tab switches preserve WebView + ScrollView state. Source pane factored into `mobile/src/features/import/SourceTabView.tsx`. URL imports also get a screen-level `← Back to browser` pill next to Cancel. Peach-tint "AI estimates on fractions" affordance is now image-only (URL imports read structured markup, no fraction-glyph misread risk).
- **Home clipboard banner:** If the pasteboard has a URL (`Clipboard.hasURL()` on iOS — a **presence-only check** that does NOT trigger the iOS paste permission prompt), a **compact inline banner** pinned below the search row offers **Paste**. Reading the actual URL (`Clipboard.getString()`) happens only when the user taps **Paste**, so iOS's paste permission prompt fires then, not on Home focus. Banner dedup is session-scoped: hides after Paste or dismiss, reappears on next app-foreground if the URL is still copied. Replaced the old bottom-sheet modal on 2026-04-24 after users reported the iOS "{App} pasted from …" dialog firing on every Home focus.
...
- **AI step/description summarization (Feature 5)** — schema is pre-baked (`recipes.description_summary`, `recipe_steps.summary_text` from migration 0013), but the Vision/URL prompts do not yet ask for summaries and the mobile UI does not render them. Intentionally deferred until Cook Mode (Phase 2.4) needs them.
- **Rule-based time estimator fallback** — when JSON-LD publishes prepTime + cookTime but no totalTime (e.g. savoryonline), the client derives `~total` at render time. A server-side second-pass AI fallback for sites where JSON-LD succeeds but times are partial was scoped but not built.
...
```
Input (image or URL)
  → Optimize (sharp: auto-orient, resize ≤3072px, JPEG 85% for storage; separate q90 OCR re-encode removed 2026-04-19 as redundant)
  → Parse (images: single gpt-4o call with merged schema, temp=0 strict JSON schema, detail:high, max_tokens:4500; URLs: JSON-LD/Microdata/DOM/gpt-5.4 AI cascade with retry, quality gate, and extraction logging)
