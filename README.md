...
- **Split-call image parsing** (shipped 2026-04-19, PR #3 + #5): two parallel OpenAI calls merged client-side. **Call A** uses `gpt-5.4` Vision at `detail:"high"` + 3072px + `temperature: 0` for accuracy-critical fields (title, servings, ingredients + fractions, metadata, page-level signals). **Call B** uses `gpt-4o` with an explicit "≤40-word, preserve every numeric/time/temp/tool" concision rule for steps + description. Both use strict JSON schema (`response_format: { type: "json_schema", strict: true }`). `Promise.allSettled` → total latency = `max(A, B)`. Partial-success flow: Call A fails → error candidate, Call B fails → candidate saves with `extractionError:"steps_failed"` + `STEPS_EXTRACTION_FAILED` FLAG banner. Measured: single-page p50 ~14s / p95 ~18s (was 30-45s pre-split). **Fraction verification UX**: non-integer ingredient amounts render with a `LIGHT_PEACH` tint + one-time banner to compensate for residual ~10% deterministic LLM fraction misreads on some cookbook fonts. See [`docs/PARSING_KNOWN_LIMITATIONS.md`](docs/PARSING_KNOWN_LIMITATIONS.md) → "Image parsing."
...
Input (image or URL)
  ...
  → Parse (images: split-call — gpt-5.4 ingredients + gpt-4o steps in parallel, both temp=0 strict JSON schema; URLs: JSON-LD/Microdata/DOM/gpt-5.4 AI cascade with retry, quality gate, and extraction logging)
